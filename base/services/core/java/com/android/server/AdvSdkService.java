...
public final class AdvSdkService extends IAdvSdkService.Stub {
	...
	private static final String LSOCK_NAME = "advsock";
	...
	/**
	  * for low-level function begin by jin.xin
	**/
	private boolean connect() {
        ...
        try {
            ...
            LocalSocketAddress address = new LocalSocketAddress(LSOCK_NAME,
	...
	private void cpyinfo(byte dst[], byte src[], int len){
    	int i;
    	dst[0] = (byte)len;
    	for(i=1; i<len+1; i++)
        	dst[i] = src[i];
	...
	public boolean startVncServer(String passwd){
		int ret = -1;
		StringBuilder builder = new StringBuilder("startvncserver");
        builder.append(' ');
    	builder.append(passwd);
    	ret = execute(builder.toString());
    	if(ret != 0){
       		Slog.i(TAG, "network error\n");
			return false;
    	}
		if(buf[0] == RET_OK)
			return true;
	...
	/** 
	  * for low-level function end by jin.xin
	**/
